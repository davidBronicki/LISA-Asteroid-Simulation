...
def fourier(List, dx, maxFrequency = 0):
    dF = 1 / (dx * len(List))
    # newList = np.fft.rfft(List) / np.sqrt(len(List))
    newList = np.fft.rfft(List)
    F = np.arange(0, len(newList) * dF - dF / 10, dF)
    if maxFrequency == 0:
        return F, newList
    else:
        n = int(maxFrequency / dF)
        return F[:n], newList[:n]

# def invFourier(List, dx, maxFrequency = 0):
#     dF = 1 / (dx * len(List))
#     newList = np.fft.rfft(List) / np.sqrt(len(List))
#     F = np.arange(0, len(newList) * dF - dF / 10, dF)
#     if maxFrequency == 0:
...
# def fourier(List, dx):
#     dF = 1 / (dx * len(List))
#     newList = np.fft.fft(List) / np.sqrt(len(List))
#     F = np.arange(-(len(newList) * dF - dF / 10), len(newList) * dF - dF / 10, dF)
#     if len(F) != len(newList):
#         del F[0]
#     return F, newList

# def invFourier(List, dx):
#     dF = 1 / (dx * len(List))
#     newList = np.fft.fft(List) / np.sqrt(len(List))
#     F = np.arange(-(len(newList) * dF - dF / 10), len(newList) * dF - dF / 10, dF)
#     if len(F) != len(newList):
