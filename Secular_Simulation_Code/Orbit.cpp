...
:
	...
	name(inName)
{
	if (isMeanAnom)//true means mean anomaly was passed as "anomaly"
	{
		meanAnomaly = doubleMod(anomaly, TWOPI);
	}
	else//false means true anomaly was passed as "anomaly"
	{
		trueAnomaly = doubleMod(anomaly, TWOPI);
		trueToEccAnom();
		eccToMeanAnom();
	}
	if (sInc == 0)//divided by zero issue if sInc=0
	{
		sInc = 1e-7;
	}
